...
with open("input_10.txt") as f:
    for line in f.readlines():
        ...
        problems.append((b[1:], num_switches, switches))
...
def solve_problem(p):
    ...
    for i, l in enumerate(sw):
        presses = model.new_int_var(0, 1000, f"n{i}")
        num_presses.append(presses)

    for j, b in enumerate(b):
        t = [num_presses[i] for i, s in enumerate(sw) if j in s]
        total = model.new_int_var(0, 1000 * len(t), f"sum_{j}")
        model.add(total == sum(t))
        eq = model.add_modulo_equality(
            1 if b == "#" else 0,
            total,
            2,
        )
